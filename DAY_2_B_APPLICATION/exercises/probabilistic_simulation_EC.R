...
# Question
# If two doses are taken into Phase 3, 
# one for patients < 70 kg and one for patients >= 70 kg, 
...
demo <- read.table("demographics.csv",header=T,sep=",") %>% filter(WT < 70) %>% sample_n(1000)
...
sim <- function(i,data,des) {
  ...
  mod %>% carry_out(dose) %>% mrgsim(data=data,tgrid=des,obsonly=TRUE) %>% mutate(irep=i) 
}
