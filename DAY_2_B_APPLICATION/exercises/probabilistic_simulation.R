...
# Question
# If a 400 mg Q2W dosing regimen is taken into Phase 3, 
# what is the probability that an individual patient 
# will achieve a 40% reduction in NTX at trough?
...
sim <- function(i,data,des) {
  ...
  mod %>% Req(PCFB) %>% mrgsim(data=data,tgrid=des,obsonly=TRUE) %>% mutate(irep=i) 
}
