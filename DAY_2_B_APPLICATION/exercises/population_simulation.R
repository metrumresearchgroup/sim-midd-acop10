...
# Question
# What is the probability that at least a 43% mean reduction in NTX 
# would be observed at steady-state in the OPG-treated study arm?
...
post <- read.table("post.csv",header=T,sep=",") %>% sample_n(1000)
...
demo <- read.table("demographics.csv",header=T,sep=",") %>% sample_n(600)
...
sim.mean <- function(i,data,des) {
  ...
  res <- mod %>% Req(PCFB) %>% mrgsim(data=data,tgrid=des,obsonly=TRUE)
...
ggplot(data=data,aes(x=THETA10,y=MEAN)) + geom_point() + geom_smooth() + xlab("Bioavailabiligy") + ylab("Mean Change in NTX (%)")
