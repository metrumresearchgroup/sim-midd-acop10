...
double CFB  = PDDV-NTX_0;
double PCFB = 100*CFB/NTX_0;
double logit = -2.57 + 0.0164*(AGE-55) - 0.00975*(WT-70) + 0.0215*PCFB;
double pFrac = exp(logit)/(1+exp(logit));
double FRAC = R::rbinom(1,pFrac);

[ capture ] @annotated
IPRED : Individual-predicted concenrtation (ng/mL)
PKDV  : Fc-OPG serum concentration (ng/mL)
PDDV  : NTX (nM BCE/mM creatinine per hr)
CFB   : Change from baseline NTX
PCFB  : Percent change from baseline NTX
FRAC  : Fracture (0/1)
